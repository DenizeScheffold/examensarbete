...
public class WeekServiceImpl implements WeekService {
    ...
    //TODO: see if user already has activity the same day
    public void solveConflict(long weekNumber, Day dayUser1, Day dayUser2) {
        int count1 = 0;
        int count2 = 0;

        Predicate<String> bi = day -> day.endsWith("GREEN");
        //  return (day.endsWith("RED"));
        ...
        while (user1.hasNext()) {
            ...
            if (bi.test(dayUser1DB.getPlanDay()))
                count1++;
            if (bi.test(dayUser2DB.getPlanDay()))
                count2++;
